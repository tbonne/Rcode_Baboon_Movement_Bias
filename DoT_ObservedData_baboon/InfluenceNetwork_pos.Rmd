---
title: "Social structure in observed positive movement bias"
...
1.)  Visualize the network of influence amoung individuals in the group (influence was estimated by Direction Matching)

```{r}
a.s <- read.csv("data/influence_net_obs_pos.csv", header=T,row.names=1)   
m.a<-as.matrix(a.s)
...
2)  Compare centrality of individuals in the group. Individuals are coded now as Male (M) or Female (F) and a rank score (e.g., Top ranking female: F1)
...
3)  Estimate the influence structure of the group using a weighted core-periphery algorithm
...
k.core.weighted <- function (graphIn){
  ...
  for (i in 1:13){
    if(i==13){
      ...
      #plot(graphIn) #to plot each removal
    }
  }
  colnames(coreTest)<-c("id","ki")
  coreTest  
...
```{r}
gg.i<-as.igraph(gg)
rdg.c<-k.core.weighted(gg.i) 
rdg.c<-rdg.c[order(rdg.c$id),]
coreInd.w <- rdg.c[,2]
gg<-qgraph((m.a),groups=factor(coreInd.w),color=c("grey30", "grey50", "grey80"),
           vsize = (alpha.centrality(gg.i)*1.5+4), legend = F)
...
for(i in 1:10000){ #this can be increased (takes a while at 10,000)
